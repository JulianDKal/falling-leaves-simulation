...
int main() {
   if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS) < 0) {
    ...
    // CRITICAL: Request DESKTOP OpenGL, not OpenGL ES
    ...
    // Add after creating context but before GLEW init
    SDL_GL_MakeCurrent(window, context);

    std::cout << "SDL_GL_GetAttribute reports:" << std::endl;
    int major, minor, profile;
    SDL_GL_GetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, &major);
    SDL_GL_GetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, &minor);
    SDL_GL_GetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, &profile);
    std::cout << "  OpenGL " << major << "." << minor << std::endl;
    std::cout << "  Profile: " << (profile == SDL_GL_CONTEXT_PROFILE_CORE ? "Core" : 
                                (profile == SDL_GL_CONTEXT_PROFILE_COMPATIBILITY ? "Compatibility" : "ES")) 
            << std::endl;

    glewExperimental = GL_TRUE;
    GLenum err = glewInit();
    //For some reason, making this check causes the application to crash. WIthout it, everything runs fine.
    // if (err != GLEW_OK) {
    //     std::cerr << "GLEW initialization failed: " << glewGetErrorString(err) << std::endl;
    //     // Clear any OpenGL error generated by glewExperimental
    //     glGetError();
